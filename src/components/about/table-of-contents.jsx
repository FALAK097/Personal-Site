          ...
          <button
            ...
            className={`relative flex items-center w-full px-3 py-2 text-sm rounded-md cursor-pointer transition-all duration-300 ease-in-out ${
              activeSection === section.id
                ? "bg-clay-500/10 text-clay-500 font-medium border-l-2 border-clay-500"
                : "text-muted-foreground hover:text-clay-400 hover:bg-transparent hover:border-l-2 hover:border-border"
            }`}
          >
            <div
              className={`absolute left-1 w-2 h-2 rounded-full transition-all duration-300 ease-in-out ${
                activeSection === section.id
                  ? "bg-clay-500 scale-110"
                  : "bg-border scale-100"
              }`}
            />
